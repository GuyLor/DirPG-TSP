...
class PriorityQueue:
    ...
    def expand(self, state, logprobs):
        ...
        special_child = Node(
            first_a=self.current_node.first_a, # keep as is, size=[batch,1]
            not_visited=not_visited, # --> function of (visited, special_action) using torch.where
            prefix=self.current_node.prefix + [special_action], # --> padded prefix. size=[batch,graph_size]
            lengths=length, # keep as is, size=[batch,1]
            cur_coord=cur_coord,  # keep as is, size=[batch,1, 2] --> maybe remove depends on where we compute mst
            done=len(not_visited) == 0,  # size=[batch,1] (torch.where?)
            logprob_so_far=self.current_node.logprob_so_far + logprobs[special_action],
            max_gumbel=self.current_node.max_gumbel, # size=[batch,1]
            next_actions=not_visited,   ##### !!!! ####
            bound_togo=self.current_node.bound_togo + self.mst_edges[special_action].sum(),
            t_opt=self.current_node.t_opt)  # size=[batch,1]
        ...
        m = time.time()
        #  next_actions starts from [1,1,1,1] and scatters 0 at special_action index
        other_actions = [i for i in self.current_node.next_actions if i != special_action] #  scatter 0
